...
An open-source client/server voice and text assistant controlling a user-configured Omarchy host through configured agent runtimes. Desktop, phone, and terminal clients share one conversation that the user can speak or type in, with interactive views, explicit device handoff, parallel tasks where tools allow it, and past-conversation retrieval. Remote operation requires the configured host to be awake and online.

Implement the eight testable deliverables below. Client/server separation is foundational from deliverable 1. Each milestone needs a user acceptance demo, relevant automated checks, and an acceptance checkpoint before expanding scope. This is an implementation plan; the planning task does not authorize implementation. Run agents through user-installed Codex, Claude Code, or OpenCode CLIs, reusing their authentication; document prerequisites and detect missing setup rather than assuming a particular developer's environment. Users need only configure the agent runtimes they intend to use; the voice model is always required, even for text-only use.
...
| Client | Desktop, phone, or terminal (TUI) app. Desktop and phone clients provide text and voice interaction and views; the TUI provides text only. |
| TUI | Terminal client: typed input and text replies only, no views. A product client under the same one-active-client rule. |
...
| Device handoff | Explicit transfer of the conversation and its view state together to another client, including to or from the TUI. |
| Voice model | GPT-Live, responsible for typed and spoken interaction and delegation. |
| Input mode | How the active client takes input: typing is always available; the microphone is off, hold-to-speak, or hands-free. |
| Reply mode | Whether Mia delivers replies as text or voice; a per-device preference. The TUI is fixed to text. |
...
| Hide view | Dismiss visuals while leaving the conversation and tasks running. |
...
- Every client accepts typed input; desktop and phone clients also take speech: hold to speak, release to submit, or optional hands-free conversation mode. Reply mode (text or voice) is independent of input mode. Input and reply mode are remembered per device and not carried by device handoff, so a handoff never opens a microphone or starts speech the user did not choose on that device.
- Voice replies show no live transcript. When the user switches reply mode to text, the client may show the transcripts of earlier voice replies. This presentation is expected to change: the server always records reply transcripts, and whether and when a client shows them is a client display decision, not a protocol or server behavior.
- Speaking interrupts: it stops playback and blocks new consequential actions while listening. Submitting typed input is not itself an interruption; GPT-Live decides whether to stop speaking. Every client, the TUI included, has an explicit interrupt control with the guarantees of a spoken interruption. Handle in-flight actions according to actual cancellation support; never imply completed actions were undone.
- Mia's agents know the active client and its capabilities (input and reply mode, views, workspace control) and shape their replies to them; for example, they answer the TUI in text instead of offering a view.
...
- Provide an app-style singleton desktop window and phone views; the TUI shows no views. Mia decides when to open, update, or remove a view; the user influences views by asking and by interacting with visible content. The active agent can use a configured Hyprland MCP integration to move/fullscreen the host window. Hide view dismisses visuals without stopping the conversation or tasks; “close the view” has the same meaning. Close Mia shuts the client. When asked to show an already visible view, report that it is visible rather than creating another window. If it is on another workspace, offer to switch when Hyprland MCP or equivalent window control is available; otherwise explain the limitation.
...
Add the voice model (GPT-Live) as the conversational owner of typed and spoken input, and a desktop client with typing, hold-to-speak/release-to-submit, hands-free conversation, text/voice reply mode, concise spoken results, and an explicit interrupt control. Promote the D1 text client to the TUI, a product client whose input also goes through GPT-Live.
...
**Pass:** Substantive questions reach the active agent whether typed or spoken, in one conversation. Text reply mode produces no speech; each device's input and reply mode persist across restarts. The explicit interrupt, in the desktop client and the TUI, gives the guarantees of a spoken interruption. No stale playback after interruption and no microphone transmission outside the chosen listening mode. Verify no new consequential action starts while a correction is being heard. Record audio, transcripts, actual playback, and observable timing without credentials or sensitive approval content.
...
Extend the working voice, text, view, approval, and task-notification journey to a phone client, extending D3's device handoff to the phone in both directions.

**User acceptance test:** Start on desktop, open phone, decline then accept device handoff, continue by voice and by typing with views, and transfer back. Approve/reject a pending call after handoff. Close during a task and reopen from a phone notification. Exercise connection loss and host unavailability.

**Pass:** One active conversation and client, with input, replies, and views moving together and no duplicate agents/actions; the phone keeps its own input and reply mode. Only authorized clients can connect; no unsolicited microphone activation or silently queued consequential commands. Conversation, visual state, and pending approvals remain correctly associated. Phone supports showing/hiding views; workspace controls apply to desktop only.
