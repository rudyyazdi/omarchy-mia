...
| 5 | `apps/server/src/engine.ts` | Read top to bottom: `decide`/`commit`/`perform` plumbing (a transition's records, effects and next state are built in `transition-draft.ts`; every change to a conversation, its start included, is decided purely in `decide-conversation.ts`), then the six commands, then `onRuntimeEvent`, `handlePermission`, `finishTurn`. |
...
In the code: `engine.submitText` → `engine.startTurn` → `adapter.submitTurn` → `onRuntimeEvent("text_delta")` → `emit("text_delta")`. Now
