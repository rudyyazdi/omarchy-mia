...
**F1: SIGTERM causes a duplicate tool dispatch (mitigated).** On SIGTERM, the runtime runs a graceful shutdown that closes its MCP connections ("HTTP connection closed ... (cleanly)"), its tool layer treats the closure as an expired session ("MCP session expired during tool call ... Retrying tool 'slow' after session recovery") and re-sends the in-flight tool call once, **without consulting the permission tool again**. The fixture recorded two `entered` events for one approval in both SIGTERM runs (`13-49-05`, `13-56-46`, `13-59-33`). With an uncancellable tool this would double the effect. Mitigation: the adapter interrupts with **SIGKILL**, which leaves no user-space code to retry; the SIGKILL run shows exactly one `tools/call`. The same retry can be triggered by any connection loss during a call, outside Mia's control; this is recorded as a runtime limitation for production MCP servers.
