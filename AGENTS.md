...
- Synchronous I/O (`*Sync`) runs only before a process starts serving or in a one-shot command, never while a server or client is serving, because it stalls every connection. The SQLite catalog is the exception: `node:sqlite` is synchronous only, so keep each statement short.
