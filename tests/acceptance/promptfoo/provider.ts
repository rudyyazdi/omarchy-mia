...
export default class MiaScenarioProvider {
  ...
  constructor(options: ProviderOptions = {}) {
    ...
    this.promptVersion = options.config?.promptVersion ?? process.env.MIA_AGENT_PROMPT_VERSION ?? "agent-v1";
  }
  ...
  async callApi(_prompt: string, context?: { vars?: Record<string, unknown> }): Promise<{ output: string; error?: string; format?: string }> {
    ...
    const secretVar = scenario.profile === "fixture-test" ? "MIA_SECRET_FILE_1" : "MIA_SECRET_FILE_2";
    const url = process.env[urlVar];
    const secretFile = process.env[secretVar];
    const harnessUrl = process.env.MIA_FIXTURE_HARNESS_URL;
    if (!url || !secretFile || !harnessUrl) return { output: "", error: `harness environment missing (${urlVar}, ${secretVar}, MIA_FIXTURE_HARNESS_URL)` };
    const budget = LiveCallBudget.fromEnv(resolve(process.env.MIA_REPO_ROOT ?? ".", ".mia-state/live-calls.jsonl"));
    const clientId = `pf_${scenarioName}_${Date.now().toString(36)}`;
    const connect = async () => {
      const c = new MiaClient({ url, secret: MiaClient.readSecret(secretFile), clientId, build: { name: "promptfoo-provider", version: "0.1.0", commit: null, dirty: null } });
      await c.connect();
      return c;
    };
    ...
    try {
      ...
      return { output: JSON.stringify({ ...evidence, prompt_version: this.promptVersion }), format: "json" };
    } catch (error) {
      return { output: JSON.stringify({ scenario: scenarioName, conversation_id: client.conversationId, error: error instanceof Error ? error.message : String(error), events: client.events.map((e) => ({ type: e.type, payload: e.payload })) }), error: error instanceof Error ? error.message : String(error) };
    } finally {
      client.close();
